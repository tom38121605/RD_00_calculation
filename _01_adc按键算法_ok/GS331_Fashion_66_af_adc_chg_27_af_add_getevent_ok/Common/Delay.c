...
void Timer0_Delay10us(UINT32 u32CNT)
{
    ...
    TIMER0_MODE1_ENABLE;      //TMOD |= 0x01;                         		  //Timer0 is 16-bit mode

    set_TR0;                            		    //Start Timer0
    while (u32CNT != 0)
    {
        TL0 = LOBYTE(TIMER_DIV12_VALUE_10us);	//Find  define in "Function_define.h" "TIMER VALUE"
...
void Timer0_Delay1ms(UINT32 u32CNT)
{
   ...
   //TMOD |= 0x01;                           		//Timer0 is 16-bit mode
   TMOD&=0xf0; TMOD |= 0x01;                     //Timer0 is 16-bit mode
   ...
   while (u32CNT != 0)
   {
      ...
      TL0 = TIMER_DIV12_VALUE_1ms & 0xff;          
      TH0 = TIMER_DIV12_VALUE_1ms >> 8 ;    
